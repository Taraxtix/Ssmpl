	...
	pub fn simulate(&mut self) {
		...
		while ip < self.ops.len() {
			match &self.ops[ip] {
				...
				| Op::Dump(_) => match stack.pop().unwrap() {
					| Data::I64(i) | Data::Ptr(i) => println!("{}", i),
					| Data::F64(f) => println!("{:?}", f),
					| Data::Bool(b) => println!("{}", b),
				},
				| Op::Add(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					...
					| (Data::F64(v1), Data::F64(v2)) => stack.push(Data::F64(v2 + v1)),
					| (a, b) => stack.push(Data::I64(b.to_i64() + a.to_i64())),
				},
				| Op::Sub(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (Data::F64(v1), Data::I64(v2)) => {
						stack.push(Data::F64(v2 as f64 - v1))
					},
					| (Data::I64(v1), Data::F64(v2)) => {
				...
				| Op::Mul(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (Data::F64(v1), Data::I64(v2)) => {
						stack.push(Data::F64(v2 as f64 * v1))
					},
					| (Data::I64(v1), Data::F64(v2)) => {
				...
				| Op::Div(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (Data::F64(v1), Data::I64(v2)) => {
						stack.push(Data::F64(v2 as f64 / v1))
					},
					| (Data::I64(v1), Data::F64(v2)) => {
				...
				| Op::Increment(_) => match stack.pop().unwrap() {
					| Data::F64(f) => stack.push(Data::F64(f + 1.)),
					| a => stack.push(Data::I64(a.to_i64() + 1)),
				},
				| Op::Decrement(_) => match stack.pop().unwrap() {
					| Data::F64(f) => stack.push(Data::F64(f - 1.)),
					| a => stack.push(Data::I64(a.to_i64() - 1)),
				},
				| Op::Mod(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (a, b) => stack.push(Data::I64(b.to_i64().rem_euclid(a.to_i64()))),
				},
				| Op::Drop(n, _) => {
					for _ in 0..*n {
						stack.pop();
				...
				| Op::Eq(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
				...
				| Op::Neq(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (Data::F64(val_r), Data::F64(val_l)) => {
						stack.push(Data::Bool(val_l != val_r))
					},
					| (a, Data::F64(val_l)) => {
				...
				| Op::Lt(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (Data::F64(val_r), Data::F64(val_l)) => {
						stack.push(Data::Bool(val_l < val_r))
					},
					| (a, Data::F64(val_l)) => {
				...
				| Op::Gt(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (Data::F64(val_r), Data::F64(val_l)) => {
						stack.push(Data::Bool(val_l > val_r))
					},
					| (a, Data::F64(val_l)) => {
				...
				| Op::Lte(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (Data::F64(val_r), Data::F64(val_l)) => {
						stack.push(Data::Bool(val_l <= val_r))
					},
					| (a, Data::F64(val_l)) => {
				...
				| Op::Gte(..) => match (stack.pop().unwrap(), stack.pop().unwrap()) {
					| (Data::F64(val_r), Data::F64(val_l)) => {
						stack.push(Data::Bool(val_l >= val_r))
					},
					| (a, Data::F64(val_l)) => {
				...
				| Op::Syscall(syscode, _argc, annot) => match simulate_syscall(syscode) {
