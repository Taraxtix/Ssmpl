...
impl<'a> Program<'a> {
	...
	pub fn type_check(mut self) -> Self {
		...
		ops.iter_mut().for_each(|op| match op {
			...
			| Op::Mod(annot1, annot2) => match (stack.pop(), stack.pop()) {
				| (Some(a), Some(b)) => {
					let a_typ = a.get_type().unwrap();
					let b_typ = b.get_type().unwrap();
					self.check_implicit_conversion(&a, &Type::I64);
			...
			| Op::Increment(annot) | Op::Decrement(annot) => match stack.pop() {
				| Some(a) => match a.get_type().unwrap() {
					| Type::F64 => {
				...
				if stack.len() < *n as usize {
					self.add_error(format!(
						"Cannot drop more elements than there are in the stack.\n Tried \
						 to drop {} elements, but there's only {} available on the stack",
						n,
						stack.len()
					))
					.exit(1);
			...
			| Op::Swap(_) => match (stack.pop(), stack.pop()) {
				| (Some(a), Some(b)) => {
					stack.push(a);
					stack.push(b);
				},
			...
			| Op::Over(n, annot) => {
				if stack.len() < *n as usize + 1 {
					self.add_error(format!(
						"Cannot get over {} elements because there's only {} available \
						 on the stack",
						n,
						stack.len()
					))
			...
			| Op::Dup(n, annot) => {
				if stack.len() < *n as usize {
					self.add_error(format!(
						"Cannot copy {} element because there's only {} available on \
						 the stack",
						n,
						stack.len()
					))
			...
			| Op::Then(label_count, _, a) => {
				if let Some(Op::If(if_label_count, _)) = cf.pop() {
					*label_count = *if_label_count;
					...
					match stack.pop() {
						| Some(a) => {
							...
							if stack_snapshot.clone() != stack {
								self.add_error(format!(
									"{}: Condition between If and Then must only add \
									 one value to the stack",
									a.get_pos()
								))
								.exit(1);
							}
					...
					self.add_error(format!("{}: Expected If before Then", a.get_pos()))
						.exit(1);
				}
			},
			...
			| Op::End(label_count, while_, a) => match cf.pop() {
				| Some(Op::Then(then_label_count, ..)) => {
					*label_count = *then_label_count;
					let stack_snapshot = stack_snapshots.pop().unwrap();
					if stack_snapshot != stack {
						self.add_error(format!(
							"{}: The code inside a IF ... THEN ... END block should not \
							 alter the stack\nBefore: {}\nAfter: {}",
							a.get_pos(),
				...
				| Some(Op::Else(else_label_count, ..)) => {
					*label_count = *else_label_count;
					...
					if stack_snapshot != stack {
						self.add_error(format!(
							"{}: code inside both of IF ... THEN ... ELSE ... END \
							 blocks should alter the stack in the same way\nThen: \
							 {}\nElse: {}",
							a.get_pos(),
							Stack::from_vec(stack_snapshot),
							Stack::from_vec(stack.clone())
				...
				| Some(Op::Do(do_label_count, ..)) => {
					...
					*while_ = true;
					...
					if stack_snapshot != stack {
						self.add_error(format!(
							"{}: code inside of WHILE ... DO ... END block should not \
							 alter the stack\nBefore: {}\nAfter: {}",
							a.get_pos(),
							Stack::from_vec(stack_snapshot),
							Stack::from_vec(stack.clone())
					...
					*label_count = *while_label_count;
					...
					match stack.pop() {
						| Some(a) => {
							...
							if stack_snapshot.clone() != stack {
								self.add_error(format!(
									"{}: Condition between While and Do must only add \
									 one value to the stack",
									a.get_pos()
								))
								.exit(1);
					...
					self.add_error(format!("{}: Expected While before Do", a.get_pos()))
						.exit(1);
				}
			},
